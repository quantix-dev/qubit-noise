...
type NoiseMap = {[number]: number}
type GradientMap = {[number]: number, max: number, min: number}
type GenSettings = {
    width: number?, 
    height: number?, 
    seed: number?, 
...
local function normalise(_value: number, _min: number, _max: number): number
...
@returns <Signal> noiseAdded [returns: <2D Partial NoiseMap>]
]]
function noise.Generate2DPartialMap(_width: GenSettings | number?, _height: number?, _seed: number?, _scale: number?, _octaves: number?, _persistence: number?, _lacunarity: number?, _offset: Vector2?, _normalise: boolean?): Signal
	...
	local elementAdded, generationFinished = Signal.new(), false
	...
	return elementAdded, generationFinished
end
...
function noise.generate2DNoiseMap(...): NoiseMap
	local args = {...}
...
function noise.generateRadialGradient(_width: number, _length: number, _doNormalisation: boolean): GradientMap
	local gradients = {}
